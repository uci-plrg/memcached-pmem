...
static void stats_init(void) {
    ...
    if (settings.pslab_recover == true)
        process_started = pslab_process_started(process_started);
#endif
    stats_prefix_init();
}
...
#ifdef PSLAB
    pslab_update_flushtime(settings.oldest_live);
#endif
...
#ifdef PSLAB
            if (settings.pslab_recover == true)
                monotonic_start -= pslab_process_started(0) - process_started;
#endif
        }
#endif
    }
...
#endif
#ifdef PSLAB
           "   - pslab_file:          file to write to for pmem slab pool.\n"
           "   - pslab_size:          pslab file size to allocate.\n"
           "   - pslab_policy:        pslab allocation policy.\n"
           "   - pslab_recover:       do recovery from pslab.\n"
#endif
           );
    return;
}
...
static bool _parse_slab_sizes(char *s, uint32_t *slab_sizes) {
    ...
    for (char *p = strtok_r(s, "-", &b);
        ...
        slab_sizes[i++] = size;
        last_size = size;
        if (i >= MAX_NUMBER_OF_SLAB_CLASSES-POWER_SMALLEST) {
...
#endif
#ifdef PSLAB
        [PSLAB_FILE] = "pslab_file",
        [PSLAB_SIZE] = "pslab_size",
        [PSLAB_POLICY] = "pslab_policy",
    ...
    while (-1 != (c = getopt(argc, argv, shortopts))) {
       ...
       case 'X' :
        ...
        case 'o': /* It's sub-opts time! */
            ...
            case PSLAB_SIZE:
                if (subopts_value == NULL) {
                    fprintf(stderr, "Missing pslab_size argument\n");
                    return 1;
                }
...
fprintf(stderr, "pslab_size=%ld", pslab_size);
            ...
            case PSLAB_POLICY:
                ...
                } else if (strncmp(subopts_value, "balanced", 8) == 0) {
                    settings.pslab_policy = PSLAB_POLICY_BALANCED;
                } else {
                    fprintf(stderr, "unknown pslab policy\n");
                        return 1;
                }
                break;
            case PSLAB_RECOVER:
                settings.pslab_recover = true;
                /* no pslab_size and use_slab_sizes */
                break;
            case PSLAB_FORCE:
                pslab_force = true;
                break;
    ...
    if (pslab_file) {
        if (settings.pslab_recover == true) {
            if (pslab_pre_recover(pslab_file, slab_sizes,
                ...
                if (slab_sizes_unparsed != NULL) {
                    fprintf(stderr,
                        "Cannot use slab_sizes with pslab recovery enabled\n");
                    exit(EX_USAGE);
                }
                if (pslab_size) {
                    fprintf(stderr,
                        "Cannot use pslab_size with pslab recovery enabled\n");
                    exit(EX_USAGE);
                }
                //TBD: more checks for reboot settings inconsistencies
                use_slab_sizes = true;
            ...
            if (pslab_size)
                ...
                settings.pslab_size = 64 * 1024 * 1024; /* default is 64MB */
    ...
    if (pslab_file) {
        ...
        int dump_num = slabs_dump_sizes(dump_sizes, MAX_NUMBER_OF_SLAB_CLASSES);
        if (dump_num == -1) {
        ...
        if (settings.pslab_recover == false) {
            if (pslab_create(pslab_file, settings.pslab_size,
                ...
                fprintf(stderr, "pslab_init failed\n");
                exit(EXIT_FAILURE);
            }
        } else {
            /* do recovery after slabs_init() and stats_init() */
            for (int i = 0, tail = 0; i < dump_num + 1; i++) {
                if (slab_sizes[i] == 0 && tail == 0) {
                    slab_sizes[i] = slab_sizes[i + 1];
                    slab_sizes[i + 1] = 0;
		    tail = 1;
                }
                if (slab_sizes[i] != dump_sizes[i]) {
                    fprintf(stderr, "pslab recover failed slab sizes mismatch\n");
                    exit(EXIT_FAILURE);
                }
            }
            if (pslab_do_recover() == -1) {
                fprintf(stderr, "pslab recover failed\n");
                exit(EXIT_FAILURE);
            }
            settings.pslab_recover = false;
        }
